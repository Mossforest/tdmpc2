...
class Buffer():
    ...
    def _init(self, tds):
        ...
        return self._reserve_buffer(
            ...
            LazyMemmapStorage(self._capacity, device=torch.device(storage_device))
        )
...
class DistributedBuffer():
    ...
    def _prepare_batch(self, td):
        ...
        action = td['a'][1:]
        reward = td['r'][1:].unsqueeze(-1)
        next_s_samples = td['next_s_samples'][1:]
        r_samples = td['r_samples'][1:]
        task = td['task'][0] if 'task' in td.keys() else None
        return self._to_device(obs, action, reward, next_s_samples, r_samples, task)
